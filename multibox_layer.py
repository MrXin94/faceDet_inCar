...
class MultiBoxLayer(nn.Module):
	...
	def __init__(self):
		...
		#self.loc_landmarks_layers = nn.ModuleList()
		for i in range(len(self.in_planes)):
			self.loc_layers.append(nn.Conv2d(self.in_planes[i],self.num_anchors[i]*4, kernel_size=3,padding=1))
			#self.loc_landmarks_layers.append(nn.Conv2d(self.in_planes[i],self.num_anchors[i]*10, kernel_size=3,padding=1))
	...
	def forward(self,xs):
		...
		for i,x in enumerate(xs):
			...
			#landmarks
			#y_loc_landmark = self.loc_landmarks_layers[i](x)
			#N = y_loc_landmark.size(0)
			#y_loc_landmark = y_loc_landmark.permute(0,2,3,1).contiguous()
			#y_loc_landmark = y_loc_landmark.view(N,-1,10)
			#y_loc_landmarks.append(y_loc_landmark)
		...
		#loc_landmarks_preds = torch.cat(y_loc_landmarks,1)
